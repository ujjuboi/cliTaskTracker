  ...
  rl.question('', (input) => {
    if (0 < Number(input) && Number(input) < data.length + 1) {
      rl.question('Type in the edits: ', (taskEdit) => {
        ...
        readTasks().then(val => console.log(boxen(val, { title: "After Edit", titleAlignment: "center", padding: 1 })));
        console.log(chalk.whiteBright.bgBlueBright("Saved successfully."));
        return rl.close();
      })
    } else {
...
switch (process.argv.slice(2)[0]) {
  case "-a":
    console.log(boxen('Enter you tasks seperated with commas "," type "q" to exit: ', { title: "Create Mode", titleAlignment: "center", padding: 1 }));
    addTasks(rl);
    break;
  case "-r":
    ...
    if (tasks === undefined)
      tasks = '';
    console.log(boxen(tasks, { title: "Read Mode", titleAlignment: "center", padding: 1 }));
    rl.close();
    break;
  case "-d":
    console.log(boxen(['Enter the index of the task you want to', chalk.redBright('delete'), 'type "q" to exit: '].join(' '), { title: "Delete Mode", titleAlignment: "center", padding: 1 }));
    deleteTasks();
    break;
  case "-m":
    console.log(boxen(['Enter the index of the task you want to', chalk.blueBright('mark'), 'as done type "q" to exit: '].join(' '), { title: "Mark Mode", titleAlignment: "center", padding: 1 }));
    markTasks();
    break;
  case "-u":
    console.log(boxen(['Enter the index of the task you want to', chalk.greenBright('unmark'), 'type "q" to exit: '].join(' '), { title: "UnMark Mode", titleAlignment: "center", padding: 1 }));
    unMarkTasks();
    break;
  case "-w":
    console.log(boxen(['Enter the index of the task you want to', chalk.yellowBright('update')].join(' '), { title: "Edit Mode", titleAlignment: "center", padding: 1 }));
    editTasks();
    break;
  default:
    let renderMan = '';
    ...
    console.log(boxen(renderMan, { title: 'Manual/Commands', titleAlignment: 'center', padding: 1 }));
    rl.close();
    break;
}
